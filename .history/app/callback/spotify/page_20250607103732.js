...
export default function SpotifyCallback() {
  ...
  useEffect(() => {
    const code = new URLSearchParams(window.location.search).get('code');
     ('SpotifyCallback page: Received code from URL:', code);
    if (code) {
       ('SpotifyCallback page: Calling handleSpotifyCallback with code...');
      handleSpotifyCallback(code).then(() => {
         ('SpotifyCallback page: handleSpotifyCallback finished, redirecting to /account');
        router.push('/account');
      }).catch(error => {
