...
const LikedSongs = () => {
  const { isSpotifyAuthenticated, likedSongs, dataLoaded, fetchLikedSongs, playSpotifyTrack } = useSpotify()
        ...
        <button 
          ...
          onClick={() => {
            if (likedSongs.length > 0) {
              // Play the first song in the liked songs list
              playSpotifyTrack(likedSongs[0].track);
            }
          }}
          ...
          <tbody>
            {likedSongs.map((item, index) => (
              <tr 
                ...
                onClick={() => playSpotifyTrack(item.track)}
              >
                <td className="px-4 py-3 text-white/60">{index + 1}</td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    {item.track.album.images && item.track.album.images.length > 0 ? (
                      <div className="w-10 h-10 relative flex-shrink-0 group-hover:opacity-80">
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center z-10 rounded transition-opacity">
                      ...
                      <div className="w-10 h-10 bg-white/10 rounded flex-shrink-0 flex items-center justify-center">
                        <Play size={16} className="text-white opacity-0 group-hover:opacity-100" />
                      </div>
                    )}
                    <div>
