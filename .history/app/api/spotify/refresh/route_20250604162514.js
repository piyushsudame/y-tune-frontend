...
import { auth } from '@clerk/nextjs/server';import { auth, currentUser } from '@clerk/nextjs/server'
...
export async function POST() {
  try {
    ...
    const { userId } = auth();
    console.log('[Spotify Refresh] User ID from auth:', userId);
    
    if (!userId) {
      console.error('[Spotify Refresh] No user ID found in auth');
