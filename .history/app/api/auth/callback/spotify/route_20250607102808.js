...
export async function GET(request) {
  ...
  try {
    ...
    console.log('Callback received for user:', userId);
    ...
    // Log the values being used for token exchange
    ...
    console.log('Calculated session expiresAt:', expiresAt);

    console.log('Attempting to find and update/create SpotifySession for userId:', userId);
    console.log('Session data being saved/updated:', {
      userId,
      accessToken: tokenData.access_token ? '[PRESENT]' : '[MISSING]', // Avoid logging full token
      refreshToken: tokenData.refresh_token ? '[PRESENT]' : '[MISSING]', // Avoid logging full token
      expiresAt,
    });
    ...
    if (session) {
      console.log('Successfully saved/updated session details (excluding tokens):', {
