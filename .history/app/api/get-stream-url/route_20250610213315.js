...
console.log('Video info fetched. Available formats:', info.formats.length);

// Choose best audio-only format
const format = ytdl.chooseFormat(info.formats, { quality: 'highestaudio' });

if (!format || !format.url) {
    console.error('No streamable format found');
    return Response.json({ error: "No streamable format found" }, { status: 500 });
}
