...
export function SpotifyProvider({ children }) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setDataLoaded(prev => ({ ...prev, playlists: true }));
         ('User playlists loaded:', data.items?.length || 0, 'playlists');
      }
    } catch (error) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setDataLoaded(prev => ({ ...prev, likedSongs: true }));
         ('Liked songs loaded:', data.items?.length || 0, 'songs');
      }
    } catch (error) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setDataLoaded(prev => ({ ...prev, artists: true }));
         ('Top artists loaded:', data.items?.length || 0, 'artists');
      }
    } catch (error) {
  ...
  const loginToSpotify = () => {
    ...
    const authUrl = `https://accounts.spotify.com/authorize?client_id=${clientId}&response_type=code&redirect_uri=${encodeURIComponent(redirectUri)}&scope=${encodeURIComponent(scope)}&show_dialog=true`;
     ...
     ('Redirecting to Spotify auth URL:', authUrl);
  ...
  const handlePlay = async (songName, songInfo = null) => {
    try {
      ...
      if (songInfo) {
       ...
       ('Song playback requested:', songName);
      // Playback implementation will be added in the future
