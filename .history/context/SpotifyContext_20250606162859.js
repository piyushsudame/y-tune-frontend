...
import { getStreamUrl } from '@/utils/streamUtils';
...
export function SpotifyProvider({ children }) {
  ...
  const fetchUserPlaylists = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('User playlists loaded:', data.items?.length || 0, 'playlists');
      }
    } catch (error) {
  ...
  const fetchLikedSongs = async (limit = 50, offset = 0) => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('Liked songs loaded:', data.items?.length || 0, 'songs');
      }
    } catch (error) {
  ...
  const fetchTopArtists = async (limit = 20) => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('Top artists loaded:', data.items?.length || 0, 'artists');
      }
    } catch (error) {
  ...
  /**
   * Handle playing a song by fetching its stream URL
   * @param {string} songName - The name of the song to play
   * @param {Object} songInfo - Optional song metadata (title, artist, etc.)
   */
  const handlePlay = async (songName, songInfo = null) => {
    try {
      setIsLoadingStream(true);
      
      // Store the song info if provided
      if (songInfo) {
        setCurrentSong(songInfo);
      } else {
        // Create a basic song info object if none provided
        setCurrentSong({
          title: songName,
          artist: 'Unknown Artist',
          album: 'Unknown Album',
          image: '/album-placeholder.png'
        });
      }
      
      // Get the stream URL from the backend
      const streamUrl = await getStreamUrl(songName);
