   ...
   const handlePlay = async (songInfo) => {
    try {
      const response = await fetch(`/api/get-stream-url`, {
        ...
        body: JSON.stringify({ 
          action: 'stream',
          query: songInfo.title + ' ' + songInfo.artist,
          videoId: songInfo.id // In case videoId is directly available
        }),
      ...
      // Update song info with video details if available
      const updatedSongInfo = data.videoDetails 
        ? { 
            ...songInfo, 
            id: data.videoDetails.id,
            title: data.videoDetails.title || songInfo.title,
            artist: data.videoDetails.artist || songInfo.artist,
            thumbnail: data.videoDetails.thumbnail || songInfo.thumbnail
          } 
        : songInfo;
      
      setCurrentSong(updatedSongInfo);
      ...
      try {
        ...
        console.error('Invalid stream URL:', data.streamUrl, error);
